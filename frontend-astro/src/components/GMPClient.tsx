...
import styles from '../styles/gmp.module.css';
...
// Helper to get latest from array
            ...
            try {
                ...
                const transformedData = data.map((ipo: any) => {
                    ...
                    return {
                        ...
                        issue_price: issuePrice ? `₹${issuePrice}` : '-',
                        price: issuePrice,
                        expected_listing: estimatedListing ? `₹${estimatedListing}` : '-',
                        lot_size: ipo.lot_size ? `${ipo.lot_size} shares` : '-',
                        listing_date: ipo.listing_date || '',
                        status: ipo.status === 'open' ? 'Issue Open' :
                            ipo.status === 'upcoming' ? 'Opening Soon' : 'Closed',
                        anchor_investors: latestSub?.applications_count_qib,
                        qib_ex_anchor: latestSub?.subscription_qib,
                        non_institutional: latestSub?.subscription_nii,
                        b_nii: latestSub?.subscription_bhni,
                        s_nii: latestSub?.subscription_shni,
                        retail: latestSub?.subscription_retail,
                        employee: latestSub?.subscription_employee,
                        shareholder: latestSub?.subscription_shareholder,
                        total_subscription: latestSub?.subscription_total,
                        day_wise_subscription: undefined // This would need a separate fetch if needed
                    };
                });
